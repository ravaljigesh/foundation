...
class Form
{
    ...
    public function select2($data = array(), $options = array())
    {
        $default = [
            'label' => '',
            'options' => array(),
        ...
        $html = '<div class="'.$data['div_class'].' form-group is-focused '.($data['wrap_class'] ? $data['wrap_class'] : '').'">
        <label class="myLabel without-margin-top" for="' .$data['id']. '">' .$data['label']. '</label><select class="form-control '. $data['class'] . ' ' . ($data['multiple'] ? 'multi-select' : '') .'" name="'.$data['name'].'" id="'.$data['id'].'" '.($data['multiple'] ? 'multiple' : '').' '.$data['attribute'].'  >'.$opt.'</select></div>';
    ...
    public function select($data = array(), $options = array())
    {
        ...
        $html = '<div class="'.$data['div_class'].($data['wrap_class'] ? ' ' . $data['wrap_class'] : '').'"><label class="myLabel without-margin-top" for="'.$data['id'].'">'.$data['label'].($data['required'] ? '*' : '').'</label><select class="'.$data['class']. ' ' . ($data['multiple'] ? 'multi-select' : '') .'" name="'.$data['name'].'" id="'.$data['id'].'" '.($data['multiple'] ? 'multiple' : '').' '.$data['attribute'].'  > '.$opt.' </select></div>';
    ...
    public function radio(array $data)
    {
        ...
        $html .= '<div class="ml-1 '.($data['inline'] == true ? 'md-radio-inline' : '').' md-radio-list">';
        if ($data['label']) {
            $html .= '<label for="'.$data['name'].'" class="inline-block">'.$data['label'].'</label>';
        }

        if ($data['inline'] == true) {
            $html = '<div class="col-md-6 col-lg-6 padding-left-0">';
            $html .= '<div class="md-radio-inline inline-block md-radio-list">';
            $html .= '<label for="'.$data['name'].'" class="">'.$data['label'].'</label>';
        }

        if (count($data['options'])) {
            $html .='<div class="">';
            foreach ($data['options'] as $value => $option_name) {
                $idfor = $data['name'].$value;

                if (count($option_name)) {
                    if ($data['value_key'] && $data['value_name']) {
                        $value = $option_name[$data['value_key']];
                        $option_name = $option_name[$data['value_name']];
                    }
                }

                $html .= '<label class="inline-block"><input type="radio" id="'. $idfor .'" name="'. $data['name'] .'" class="radio" '.($value == $data['value'] ? 'checked' : '').' value="'. $value .'"><span for="'. $idfor .'"><span></span><span class="check"></span><span class="boxx"></span> '. $option_name .'</sapn></label>';
            }
            $html .='</div>';
        }
    ...
    public function mdradio($name, $options = array(), $selected_value = null, $label = null, $inline = false)
    {
        ...
        if ($inline == true) {
            $html = '<div class="col-md-4 col-lg-4 padding-left-0">';
            $html .= '<div class="md-radio-inline inline-block md-radio-list">';
